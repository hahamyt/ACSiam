...
opts = OrderedDict()
########## ALL about
opts['use_gpu'] = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")    # .device('cpu')   # torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
opts['savefig'] = True
opts['vis'] = False
opts['seed'] = 1234
########## Encoder
